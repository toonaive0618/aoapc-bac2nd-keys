...
vector<string> Notes1 = {"C",  "C#", "D",  "D#", "E",  "F",
                         "F#", "G",  "G#", "A",  "A#", "B"},
               Notes2 = {"B#", "Db", "D",  "Eb", "Fb", "E#",
                         "Gb", "G",  "Ab", "A",  "Bb", "Cb"};
const int NCNT = 12, SCNT = 8;
map<string, Scale> scales; // 主调:音阶
int Step[] = {2, 2, 1, 2, 2, 2, 1};

void genScale(int cur, vector<string> &notes, set<char> &letters, bool hasFlat,
              bool hasSharp) {
  ...
  while (cin >> sKey && sKey != "*") {
    ...
    if (sKeyValid && tKeyValid) {
      ...
      for (const auto &nt : notes) {
        ...
        if (it == ss.notes.end())
          cout << "  " << nt << " is not a valid note in the " << ss.key
               << " major scale" << endl;
        else
      ...
      cout << "Key of " << (sKeyValid ? tKey : sKey)
           << " is not a valid major key" << endl;
    }
  }
  return 0;
}

// LA 1480023	2237	A Major Problem	Accepted	C++	0.099
// 2014-07-07 13:49:06
