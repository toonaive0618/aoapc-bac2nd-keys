...
const int MAXC = 20 + 5, MAXX = 10 + 2, DX[] = {-1, 1, 0, 0},
          DY[] = {0, 0, -1, 1};
int c;

struct City {
  bool valid, complete; // 是不是一个合法的城市，是否已经完成
  int motif[MAXC], in[MAXC],
      country; // 各种硬币余额，入账的硬币个数，所在的国家
  void update() {
    assert(valid);
    _for(i, 0, c) if (motif[i] < 1) {
      complete = false;
      return;
    }
    complete = true;
...
struct Country {
  ...
  void update() {
    _rep(x, xl, xh) _rep(y, yl, yh) if (!cities[x][y].complete) {
      complete = false;
      return;
    }
    complete = true;
  ...
  for (int k = 1; cin >> c && c; k++) {
    ...
    _for(i, 0, c) {
      ...
      _rep(x, ct.xl, ct.xh) _rep(y, ct.yl, ct.yh) {
        City &ci = cities[x][y];
        ci.valid = true, ci.country = i, ci.complete = false,
        ci.motif[i] = 1000000;
      }
    }
...
// 13899646	1021	Eurodiffusion	Accepted	C++	0.236
// 2014-07-18 02:25:44
